...
#params in reservoir.dat ####
    ...
    #change parameters in irri.dat ####
    if (names(parameters)[1] %in% c("irri_loss","irri_rate_f"))        
    {
      
      target_file=paste(wasa_input_dir,"Hillslope/irri",sep="") #file that hold the parameters to be changed
      ...
      nn= which(names(parameters)=="irri_loss")
      if (length(nn)>0) #modify loss_factor
      {
        file_content[,ncol(file_content)]=parameters[1,nn]
        parameters=parameters[-nn] #remove this parameter from list
        no_params=no_params-1
      ...
      # multiply irri_rate in irri.dat by factor
      nn= which(names(parameters)=="irri_rate_f")
      if (length(nn)>0) #modify irri_rate
      {
        file_content[,c(5:8)]=file_content[,c(5:8)] * parameters[1,nn]
        parameters=parameters[-nn] #remove this parameter from list
        no_params=no_params-1
      }
      
      #re-write file
      content=paste("# Specification of irrigation operations\nsub_source	source	sub_receiver	rule	rate	rate2	rate3	rate4	loss_factor",sep = "")
      write(content, file = paste(target_file,".dat",sep=""))     #write header
      write.table(file_content, file = paste(target_file,".dat",sep=""), append = TRUE, quote = F,row.names=F,col.names=F,sep="\t", na = "")
      next
    }
