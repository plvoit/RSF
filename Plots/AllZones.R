...
barplot(Prec[,match(colnames(IrriZone1)[i],colnames(Prec))], ylim = rev(c(0,120)),yaxt = "n",
        col = rep("dodgerblue",nrow(Prec)), border = "dodgerblue", main = SubbasID_GaugeNumber[match(colnames(IrriZone1)[i],SubbasID_GaugeNumber$Subbas_ID),3], cex.main = 0.9)
axis(4, cex.axis = 0.8, at = seq(0,120,40))
mtext("Precipitation [mm]", 4, line = 2, cex.axis = 0.8, cex = 0.6)
par(new = TRUE)
plot(IrriZone1[,i]~Obs$Date, type = "l", col = "red", ylim = c(0,3500), ylab = "",
     xlab = "", cex.lab = 0.9, cex.axis = 0.8)
title(ylab = expression("Runoff [m" ^3*"/s]"), line = 2, cex.lab = 0.85, xlab = "Year")
...
for ( i in 3:ncol(IrriZone2) ){
  barplot(Prec[,match(colnames(IrriZone2)[i],colnames(Prec))], ylim = rev(c(0,120)),yaxt = "n",
          col = rep("dodgerblue",nrow(Prec)), border = "dodgerblue", main = SubbasID_GaugeNumber[match(colnames(IrriZone2)[i],SubbasID_GaugeNumber$Subbas_ID),3], cex.main = 0.9)
  axis(4, cex.axis = 0.8, at = seq(0,120,40))
  mtext("Precipitation [mm]", 4, line = 2, cex.axis = 0.8, cex = 0.6)
  par(new = TRUE)
  plot(IrriZone2[,i]~Obs$Date, type = "l", col = "red", ylim = c(0,max(IrriZone2[,i])+1000), ylab = "",
       xlab = "", cex.lab = 0.9, cex.axis = 0.8)
  title(ylab = expression("Runoff [m" ^3*"/s]"), line = 2, cex.lab = 0.85, xlab = "Year")
...
for ( i in 3:ncol(IrriZone3) ){
  barplot(Prec[,match(colnames(IrriZone3)[i],colnames(Prec))], ylim = rev(c(0,120)),yaxt = "n",
          col = rep("dodgerblue",nrow(Prec)), border = "dodgerblue", main = SubbasID_GaugeNumber[match(colnames(IrriZone3)[i],SubbasID_GaugeNumber$Subbas_ID),3], cex.main = 0.9)
  axis(4, cex.axis = 0.8, at = seq(0,120,40))
  mtext("Precipitation [mm]", 4, line = 2, cex.axis = 0.8, cex = 0.6)
  par(new = TRUE)
  plot(IrriZone3[,i]~Obs$Date, type = "l", col = "red", ylim = c(0,max(Obs[,match(colnames(IrriZone3)[i],colnames(Obs))], na.rm = T)+2000), ylab = "",
       xlab = "", cex.lab = 0.9, cex.axis = 0.8)
  title(ylab = expression("Runoff [m" ^3*"/s]"), line = 2, cex.lab = 0.85, xlab = "Year")
