#Plot the results from a WASA run. Rainfall-Runoff, residuals etc. All plots can be automatically stored in  a folder
# Copyright (C) 2020 Paul Voit

rm(list = ls())
setwd("~/Workspace/RioSaoFrancisco/ResultsCalibration/Mod")

thread <- "LakesZ1"  ## Check right subbas_id in line 11!
saveplots <- TRUE
...
mod <- read.table(paste(thread,"/thread1/Output/River_Flow.out", sep = ""), quote="\"", comment.char="", skip = 1, header = T, check.names = F)
