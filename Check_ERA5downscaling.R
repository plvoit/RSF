...
Pera <- read.csv("~/Workspace/RioSaoFrancisco/Data/xds_gerd/xds_gerd/brazil/era5/P.csv", check.names = FALSE)
...
#  plot(Pobs[,i]~Pobs$Date, type = "l", ylab= "P", main ="Daily Precipitation")
...
#for ( i in 2:ncol(Pobs_month)){
#  plot(Pres[,i]~Pres$Date, type = "l", ylab= "P",main =paste("Monthly Residuals Precipitation",names(Pobs_month[i])))
#}
...
### now with this the yearly residuals have to be calculated again, RMSE
...
### Following block is not needed, the 0's in the yearly datasets apppear because of the sum(na.rm = TRUE) function. The original dataset should be fine
### in the daily data set all the years with zero precipitation to NA
### for every  station create a vector that contains the years without precipitation
#NA_years <-  list()

#for (i in 2:ncol(Pobs_year)){
#  dummy <-  c()
#  dummy <- format(Pobs_year[Pobs_year[i] == 0,1], format = c("%Y"))
#  NA_years[[i-1]] <-  dummy
#}

## subset each station by this vector
#out_years <- NA_years[[1]]
#Pobs$year <- format(Pobs$Date, format= c("%Y"))

#Pobs[Pobs$year %in% out_years,2] <- NA


### check how the residuals are distributed for each station
#for ( i in 2:ncol(Pres_year)){
#  hist(Pres_year[,i], main = paste("Residuals", colnames(Pres_year[i])), breaks = 20)
#}
...
## export file
write.csv(Mmp_total,"Predicted_downscaling.txt", row.names = FALSE)
write.csv(Mmo_total,"Observed.txt", row.names = FALSE)
