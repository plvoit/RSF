...
irri <- read.table(paste0(thread,"IrriCal/Irri_best/Output/River_Flow.out"), quote="\"", comment.char="",skip = 1, header = T, check.names = F)
obs <- read.delim(paste0(thread,"IrriCal/init_config/Input/Time_series/discharge_obs_24.txt"), skip = 4, header = T, check.names = F)
noirri <- read.table(paste0(thread,'NoIrriCal/thread1_best/Output/River_Flow.out'), quote="\"", comment.char="",skip = 1, header = T, check.names = F)
prec <- read.csv(paste0(thread,"NoIrriCal/thread1_best/Input/Time_series/rain_daily.dat"), sep="", skip = 2, header = T, check.names = F )
...
for (i in subs){
  ...
  png(file = paste0(PaperID[match(i,subID)], "_irriNoirri.png"), bg = "white", width = 2480, height = 1748, res = 300)
  plot(df$Obs~df$Date, type = "l", ylim = c(0,max(df$irri, df$Noirri, df$Obs, na.rm = T)), cex.axis = 0.8, ylab = expression("discharge [m" ^3*"/s]"),
       xlab = "Year", cex.lab = 0.8, cex.axis = 0.8, main = paste0('streamflow gauge ', PaperID[match(i,subID)] ))
...
#### RAINFALL RUNOFF for Rio Urucuia
  ...
  irri <- read.table(paste0(thread,"2IrriCal/Irri_best/Output/River_Flow.out"), quote="\"", comment.char="",skip = 1, header = T, check.names = F)
  obs <- read.delim(paste0(thread,"IrriCal/init_config/Input/Time_series/discharge_obs_24.txt"), skip = 4, header = T, check.names = F)
  noirri <- read.table(paste0(thread,'NoIrriCal/thread1_best/Output/River_Flow.out'), quote="\"", comment.char="",skip = 1, header = T, check.names = F)
  prec <- read.csv(paste0(thread,"NoIrriCal/thread1_best/Input/Time_series/rain_daily.dat"), sep="", skip = 2, header = T, check.names = F )
  ...
  abline(v= years, lty = 2, col = "grey")
  legend("topright", legend= c("observed", "modeled (no irrigation)","modeled (irrigation)", "precipitation"),col = c("black","red", "green", "dodgerblue"), lty=1, cex=0.5)
  dev.off()
